...
public class DragComponent implements VehicleComponent {
    ...
    public void tick(Vehicle vehicle) {
        vehicle.getComponent(RigidBodyComponent.class).ifPresent(rbc -> {
            ...
            double dragForceMagnitude = 0.5 * dragCoefficient * density * crossSectionalArea * velocity.lengthSquared();
            Vector3d dragForce = new Vector3d(rbc.getVelocity()).normalize().negate().mul(dragForceMagnitude);
            if (Math.random() < 0.05) {
